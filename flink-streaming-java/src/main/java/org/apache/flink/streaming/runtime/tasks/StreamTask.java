 ...
 *  -- invoke()
 *        |
 *        +----> Create basic utils (config, etc) and load the chain of operators 创建出task相关配置，加载OperatorChain。
 *        +----> operators.setup() 执行operator的setup逻辑
 *        +----> task specific init() 执行task相关的初始化逻辑
 *        +----> initialize-operator-states() 加载并初始化operator的状态
 *        +----> open-operators() 调用各个operator的open方法
 *        +----> run() 执行各个operator内的数据处理逻辑
 *        +----> close-operators() 关闭operator
 *        +----> dispose-operators() 销毁operator
 *        +----> common cleanup 通用的清理操作
 *        +----> task specific cleanup() Task特定的清理操作
 * }</pre>
 *
...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
	 ...
	 * TODO processInput方法从上游（StreamTaskNetworkInput，InputGate）读取数据
	...
	/**
	 * TODO beforeInvoke方法主要为task的初始化操作，包含创建OperatorChain，读取上游数据和下游数据输出配置等。
	 * @throws Exception
	 */
	...
	protected void beforeInvoke() throws Exception {
		...
		// task 动作必须在 StreamTaskActionExecutor 中执行，防止出现并发执行问题，影响 checkpoint
		// 该 executor 实际为 StreamTaskActionExecutor.IMMEDIATE，即在当前线程直接运行
		actionExecutor.runThrowing(() -> {
			// 创建 SequentialChannelStateReader，用于读取checkpoint时保存的channel状态
			...
			// 获取 ResultPartitionWriter 状态
			reader.readOutputData(getEnvironment().getAllWriters(), false);
// 			// 初始化 OperatorChain 中所有的 operator
			// 调用他们的 initializeState（初始化状态）和 open（包含初始化动作）方法
	...
	/**
	 * TODO runMailboxLoop方法启动task的数据输入和处理逻辑
	 * @throws Exception
	 */
	...
	/**
	 * TODO task执行完毕后的清理工作，关闭operator等
	 * @throws Exception
	 */
	...
	protected void afterInvoke() throws Exception {
		...
		// 关闭 OperatorChain 中所有的 operator
		// 从前向后依次调用各个 operator 的close方法
