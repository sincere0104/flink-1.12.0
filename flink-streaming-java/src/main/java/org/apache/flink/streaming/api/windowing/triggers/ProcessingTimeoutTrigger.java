 ...
 * TODO 相当于 ContinuousProcessingTimeTrigger 和 ContinuousEventTimeTrigger 按照用户配置的 interval 提前触发窗口的计算，
 * TODO 因为有 resetTimerOnNewRecord 和 shouldClearOnTimeout 比他俩更灵活
 *
 * 在第一个到达的元素上，将设置可配置的处理时间超时。
 * 使用{ of（Trigger，Duration，boolean，boolean）}，
 * 您还可以通过指定{@code resetTimerOnNewRecord}重新创建每个到达元素的计时器，
 * 并且可以指定是否应通过{@code shouldClearOnTimeout}在timout上
 * 调用{@link Trigger#clear（Window，TriggerContext）}。
