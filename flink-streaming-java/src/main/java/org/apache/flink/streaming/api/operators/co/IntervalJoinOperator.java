...
public class IntervalJoinOperator<K, T1, T2, OUT>
	...
	// TODO List<BufferEntry<T1>> 对应相同时间戳的数据
	 ...
	 * 当两个流达到之后，比如左边的流有数据到达之后，就去右边的流去查找对应上下界范围内的数据
	...
	private <THIS, OTHER> void processElement(
		...
		// 获取记录的值和时间戳，判断是否延时，当到达的记录的时间戳小于水位线时，说明该数据延时，不去处理，不去关联另一条流的数据。
		if (isLate(ourTimestamp)) {
			return;
		}
        //将数据添加到对应自己流的MapState缓存状态中，key为数据的时间。
		addToBuffer(ourBuffer, ourValue, ourTimestamp);

        //去遍历另一条流的MapState，如果 ourTimestamp + relativeLowerBound <= timestamp<= ourTimestamp + relativeUpperBound ，
		//则将数据输出给ProcessJoinFunction调用，ourTimestamp表示流入的数据时间，timestamp表示对应join的数据时间
		// 假设左流10秒数据到了，between （1,5）
		for (Map.Entry<Long, List<BufferEntry<OTHER>>> bucket: otherBuffer.entries()) {
			final long timestamp  = bucket.getKey();
			// 那么下面操作就是去循环过滤右流的数据，判断右流数据的时间是否满足 10 + 1 <= left <= 10 + 5
			if (timestamp < ourTimestamp + relativeLowerBound ||
					timestamp > ourTimestamp + relativeUpperBound) {
				continue;
			}
			// 满足的话，将数据输出给ProcessJoinFunction调用
