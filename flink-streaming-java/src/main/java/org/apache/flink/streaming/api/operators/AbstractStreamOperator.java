 ...
 * TODO 对于具体的实现类，还必须实现 OneInputStreamOperator 或 TwoInputStreamOperator，用于标记算子是 一元还是二元。
 *
...
public abstract class AbstractStreamOperator<OUT>
	 ...
	 * TODO 每个算子可以有一个或多个InternalTimerService。
	 * TODO InternalTimerService的四要素是：名称、命名空间类型N（及其序列化器）、键类型K（及其序列化器），还有上文所述Triggerable接口的实现。
	 * TODO InternalTimerService经由InternalTimeServiceManager.getInternalTimerService()方法取得。
