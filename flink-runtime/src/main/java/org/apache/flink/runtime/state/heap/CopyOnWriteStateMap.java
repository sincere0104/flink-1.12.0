...
public class CopyOnWriteStateMap<K, N, S> extends StateMap<K, N, S> {
	 ...
	 * primarytable 中增量 rehash 要迁移的下一个 index
	...
	private StateMapEntry<K, N, S> putEntry(K key, N namespace) {
		...
		++modCount;
		/* TODO
		申请一个相比 primaryTable 容量大一倍的 hash 表保存到 rehashTable 中，
		慢慢地将 primaryTable 中的元素迁移到 rehashTable 中。对应到源码中：putEntry
		 方法中判断 size() > threshold 时，会调用 doubleCapacity 方法申请新的 hash
		 表赋值给 rehashTable
		 */
		if (size() > threshold) {
			doubleCapacity();
	...
	private void incrementalRehash() {
		...
		//每次至少迁移 MIN_TRANSFERRED_PER_INCREMENTAL_REHASH 个元素到新桶 默认值是4
		...
		// primaryTableSize 中减去 transferred，incrementalRehashTableSize增加 transferred
	...
	protected static class StateMapEntry<K, N, S> implements StateEntry<K, N, S> {
		 ...
		 * 用于区分不同的 Window，在 StateMapEntry 中 key 和 namespace 组合起来作为共同的主键，state 作为 value
		 */
		@Nonnull
		final N namespace;
		 ...
		 * state 表示 HashMap 中的 value，即：具体存储的数据
		 */
		@Nullable
		S state;
		 ...
		 * 表示当前 StateMapEntry 中 state （数据）更新时的版本号
		 */
		int stateVersion;
